...
// and `@pm-go/executor-claude` to their TypeScript sources so Vitest
// can resolve them without a build step. Mirrors the pattern in
  ...
  resolve: {
    alias: {
      ...
      "@pm-go/executor-claude": fileURLToPath(
        new URL("../executor-claude/src/index.ts", import.meta.url),
      ),
    },
  },
  test: {
